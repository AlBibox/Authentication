...
`;
    ...
    db.query(query, [selector, new Date()], (error, results) => {
        ...
        const hashToVerify = crypto
            ...
            .update(verifier)
            .digest('hex');
        const isVerifierValid = crypto.timingSafeEqual(Buffer.from(hashToVerify, 'hex'), Buffer.from(rows[0].verifier_hash, 'hex'));
        if (!isVerifierValid) {
        ...
        jwt.verify(refreshToken, process.env.REFRESH_SECRET, (err) => {
            if (err)
                return res.status(403).json({ message: "Firma non valida" });
            const newAccessToken = jwt.sign({ id: userId }, process.env.ACCESS_SECRET, { expiresIn: '15m' });
