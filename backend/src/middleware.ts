...
export const middlewareAutenticazione = (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const datiUtente: { id: string; email: string } = jwt.verify(token, process.env.ACCESS_SECRET as string) as { id: string; email: string };
    ...
    (req as AuthenticatedRequest).user = datiUtente; 
